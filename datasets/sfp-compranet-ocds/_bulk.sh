...
# Instala template
curl -X PUT "$ES_HOST/$ES_INDEX" -H 'Content-Type: application/json' -d'@template.json'
...
do
    ...
    ./jsonpyes/jsonpyes.py --bulk $ES_HOST --import --index $ES_INDEX --type _doc --data "$filename" > "${filename/%.lines/.log}"
done
